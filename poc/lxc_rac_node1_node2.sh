...
[ -d /container/node1 ] && exit 1 || sh $BASEDIR/step.sh sh $BASEDIR/lxc/clone_container.sh base node1
sh $BASEDIR/step.sh lxc-start --name node1 -d
[ -d /container/node2 ] && exit 1 || sh $BASEDIR/step.sh sh $BASEDIR/lxc/clone_container.sh base node2
